...
use crate::primitives::{Black, Move, MoveList, MoveType, Sides, White};
...
pub fn perft<A: AttackTable>(
    ...
    if let Some(data) = tt.probe(position.state.zobrist_key)
        && data.depth() == depth
    {
    ...
    for mv in moves.iter() {
        ...
        let is_checked = match position.turn() {
            Sides::White => move_generator.attack_table.is_checked::<Black>(position),
            Sides::Black => move_generator.attack_table.is_checked::<White>(position),
        };
        if !is_checked {
    ...
    tt.insert(position.state.zobrist_key, PerftData::new(depth, nodes));
...
// @param: depth - depth to calculate the perft for
// @return: number of leaf nodes at the given depth
pub fn perft_divide_print<A: AttackTable>(
    position: &mut Position,
    move_generator: &MoveGenerator<A>,
    tt: &mut TranspositionTable<PerftData>,
    depth: Depth,
