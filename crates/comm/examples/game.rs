...
/// DEFAULT_INTERACTIVE_SEARCH_DEPTH is the search depth used by the interactive
/// command-line game when `--depth` is not supplied
...
/// and fixed-depth `go` equivalents and displays the engine after every move
///
/// @type
pub struct InteractiveGame<EngineT> {
    ...
    search_depth: u8,    // search depth used for engine moves
}
    ...
    EngineT: UciEngine + Display,
{
        ...
        WriterT: Write,
    {
        ...
        loop {
            ...
            let result = self.search(self.search_depth)?;
...
mod tests {
    ...
    impl UciEngine for TestEngine {
        ...
        fn search(&mut self, limits: &SearchLimits) -> Result<SearchResult, Self::Error> {
            self.searches += 1;
            self.search_depths.push(limits.depth);
    ...
    fn wraps_moves_and_configured_search_depth_into_a_human_session() {
        ...
        assert_eq!(game.engine.search_depths, [Some(7), Some(1)]);
    }
