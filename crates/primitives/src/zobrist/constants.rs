...
pub const PIECE_RANDOMS: [[[ZobristKey; Square::TOTAL]; Pieces::TOTAL]; Sides::TOTAL] = {
    ...
    while raw_piece_idx < Pieces::TOTAL - 1 {
        ...
        let black_piece_idx = raw_piece_idx * 2;
        let white_piece_idx = black_piece_idx + 1;

        // set the random values for the given piece on each square per side
        let mut square_idx = 0;
        while square_idx < Square::TOTAL {
            // fetch the polyglot random from the constants table and set it in
            // our formatted randoms table with the appropriate piece offset
            let white_random = POLYGLOT_RANDOMS[white_piece_idx * Square::TOTAL + square_idx];
            randoms[Sides::White.idx()][raw_piece_idx + Pieces::Pawn.idx()][square_idx] =
                ZobristKey::new(white_random);

            // same for black
            let black_random = POLYGLOT_RANDOMS[black_piece_idx * Square::TOTAL + square_idx];
            randoms[Sides::Black.idx()][raw_piece_idx + Pieces::Pawn.idx()][square_idx] =
                ZobristKey::new(black_random);
...
pub const CASTLING_RANDOMS: [ZobristKey; Castling::TOTAL] = {
    ...
    let white_queen_random = POLYGLOT_RANDOMS[CASTLING_OFFSET + 1];
    let black_king_random = POLYGLOT_RANDOMS[CASTLING_OFFSET + 2];
    let black_queen_random = POLYGLOT_RANDOMS[CASTLING_OFFSET + 3];

    let mut i = 0;
    while i < Castling::TOTAL {
        // check each bit of the castling rights to determine what randoms to
        // combine
        let mut random = 0;
        if i & Castling::WHITE_KING.unwrap() as usize != 0 {
            random ^= white_king_random;
        }
        if i & Castling::WHITE_QUEEN.unwrap() as usize != 0 {
            random ^= white_queen_random;
        }
        if i & Castling::BLACK_KING.unwrap() as usize != 0 {
            random ^= black_king_random;
        }
        if i & Castling::BLACK_QUEEN.unwrap() as usize != 0 {
            random ^= black_queen_random;
        }

        randoms[i] = ZobristKey::new(random);
        i += 1;
    }
