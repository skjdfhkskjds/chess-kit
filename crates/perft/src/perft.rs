    ...
    TranspositionTableT: TranspositionTable<PerftData>,
{
    ...
    for mv in moves.iter() {
        ...
        // get the next eval state from the accumulator
        ...
        nodes += perft(position, move_generator, tt, accumulator, depth - 1);

        // undo the move
        position.unmake_move(mv);

        // pop the eval state from the accumulator
        accumulator.pop();
    }
    ...
    TranspositionTableT: TranspositionTable<PerftData>,
{
    ...
    for mv in moves.iter() {
        ...
        branches.push((
            mv,
            perft(position, move_generator, tt, accumulator, depth - 1),
        ));
...
where
    ...
    TranspositionTableT: TranspositionTable<PerftData>,
{
    let entries = perft_divide(position, move_generator, tt, accumulator, depth);
